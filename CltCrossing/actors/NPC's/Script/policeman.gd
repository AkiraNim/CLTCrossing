...
@onready var animation_player: AnimationPlayer = $Policeman/AnimationPlayer
...
func player_interact() -> void:
	...
	var id
	for npcs in NpcManager.npcs:
		if npcs.npc_id == npc_id and npcs.npc_name == "Policial":
			MissionManager.create_new_mission(npc_id, "Responder Duvidas", "Ajude o policial", "Livro Azul, 30 moedas")
	for missions in MissionManager.get_available_missions():
		for npcs in NpcManager.npcs:
			if npcs.npc_name == "Policial":
				id = npcs.npc_id
		if missions.npc_id == id\
		and has_dialog:
			if Dialogic.current_timeline == null:
				Dialogic.start('policial')
				has_dialog = false  # Diálogo específico
				get_viewport().set_input_as_handled()
				if !has_dialog:
					drop_npc_slot_data_by_name("Livro azul")
					PlayerManager.player.add_money(+30)
					pop_up.set_popup_text("Item recebido Livro azul\n+R$30.00", 2.0)
					
	for mission in MissionManager.get_available_missions():
		if mission.title == "Responder Duvidas":
